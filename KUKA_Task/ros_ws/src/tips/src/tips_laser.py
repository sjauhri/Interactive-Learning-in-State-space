...
from laser_env import *
from feedback_ext import *

class TIPS_laser(TIPS):
  def __init__(self, state_shape, action_shape, lr=0.00025, maxEpisodes=20, epochTrainIts=4000,  dynamicsSamples=4000, batch_size=32):
    ...
    self.ifdm_queries = 2000 # Two continous actions.
  ...
  def build_policy_model(self):
    ...
    with tf.variable_scope("policy_model") as scope:
      ...
      with tf.variable_scope("output") as scope:
        ...
        self.policy_pred_action = tf.clip_by_value(self.tmp_policy_pred_action, clip_value_min=-ACTION_LIMIT, clip_value_max=ACTION_LIMIT)
  ...
  def dynamics_sampling(self):
    ...
    Nstates = []
    Actions = []
    A = np.random.uniform(-ACTION_LIMIT, ACTION_LIMIT, self.action_dim)
    prev_time = time.time()
  ...
  def get_corrected_action(self, h_fb, state, state_corrected):
    ...
    # Continuous Actions
    Actions = np.random.uniform(-0.1, 0.1, (self.ifdm_queries,self.action_dim) )
