...
class TIPS_lunarlandercont(TIPS):
  ...
  def dynamics_sampling(self):
    ...
    for i in range(self.dynamicsSamples):
      ...
      # x-y position frame correction!
      state_mod = np.copy(state)
      prev_s_mod = np.copy(prev_s)
      state_mod[0] -= prev_s_mod[0]
      state_mod[1] -= prev_s_mod[1]
      prev_s_mod[0] = 0
      prev_s_mod[1] = 0

      States.append(prev_s_mod)
      Nstates.append(state_mod)
      Actions.append(A)
  ...
  def feedback_run(self):
    ...
    while((not terminal) and (not self.human_feedback.ask_for_done()) ):
      ...
      state_mod = np.copy(state)
      prev_s_mod = np.copy(prev_s)
      state_mod[0,0] -= prev_s_mod[0,0]
      state_mod[0,1] -= prev_s_mod[0,1]
      prev_s_mod[0,0] = 0
      prev_s_mod[0,1] = 0

      self.ExpBuff.append((prev_s_mod[0], state_mod[0], a))
